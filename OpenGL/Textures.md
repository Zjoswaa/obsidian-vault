A texture is a 2D image (though 1D and 3D textures exists) used to add detail to an object.
> [!info]
> Textures can also be used to send a large collection of arbitrary data to the shaders.
...
Texture coordinates range from `0` to `1` in the `x` and `y` axis (for 2D textures). They start at `(0, 0)` for the lower left corner a to `(1, 1)` for the upper right corner.
## Texture wrapping
...
- `GL_CLAMP_TO_BORDER` - Coordinates outside the range are not given a user-specified border color.

Each of these options can be set per coordinate axis `s`, `t` and `r` (which are equivalent to `x`, `y` and `z`) with the `glTexParameter*i:
``` CPP
...
```
## Texture filtering
- `GL_NEAREST` - Default texture filtering mode, nearest neighbor to point.
- `GL_LINEAR` - Takes an interpolated value from the texture coordinates neighboring texels.
## Mipmaps
